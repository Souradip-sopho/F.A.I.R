...
void visualise_normal(int i,pcl::PointCloud<pcl::PointXYZ> &ipCloud,pcl::PointIndices &inliers,std::string &frame,ros::Publisher &visPub,pcl::ModelCoefficients &coefficients);
/** Function Details
...
{	
	...
	{	
		...
		if(inliers.indices.size() <1)
			break;
		visualise_normal(i,tempCloud,inliers,frame,visPub,coefficients);
...
void visualise_normal(int i,pcl::PointCloud<pcl::PointXYZ> &ipCloud,pcl::PointIndices &inliers,std::string &frame,ros::Publisher &visPub,pcl::ModelCoefficients &coefficients)
{
	...
	marker.header.frame_id = frame;
	marker.header.stamp = ros::Time();
	marker.ns = "normal_estimation";
	marker.id = i;
	marker.type = visualization_msgs::Marker::ARROW;
	marker.action = visualization_msgs::Marker::ADD;
	
int count=0; 
float sum1=0,sum2=0,sum3=0,avgx=0,avgy=0,avgz=0; 
for(unsigned int k=2000; k<inliers.indices.size()-2001;k=k+50){



	float	x1=ipCloud.at(inliers.indices[k+1980]).x; 
	float	y1=ipCloud.at(inliers.indices[k+1980]).y; 
	float	z1=ipCloud.at(inliers.indices[k+1980]).z; 
	float	x2=ipCloud.at(inliers.indices[k]).x; 
	float	y2=ipCloud.at(inliers.indices[k]).y; 
	float	z2=ipCloud.at(inliers.indices[k]).z; 
	float	x3=ipCloud.at(inliers.indices[k-1980]).x; 
	float	y3=ipCloud.at(inliers.indices[k-1980]).y; 
	float	z3=ipCloud.at(inliers.indices[k-1980]).z; 
	/*float	alpha=((y1-y2)*(z2-z3)-(y2-y3)*(z1-z2))*10000; 
	float	beta=((z1-z2)*(x2-x3)-(x1-x2)*(z2-z3))*10000; 
	float	gamma=((x1-x2)*(y2-y3)-(y1-y2)*(x2-x3))*10000; 
	float   theta=sqrt(alpha*alpha+beta*beta+gamma*gamma);*/ 

	avgx=avgx+(x1+x2+x3)/3; 
	avgy=avgy+(y1+y2+y3)/3; 
  	avgz=avgz+(z1+z2+z3)/3; 
	...
	/*if(alpha<0) 
	  sum1=sum1-alpha/theta;
	else 
	  sum1=sum1+alpha/theta;
	 
	if(beta<0) 
	  sum2=sum2-beta/theta; 
	else 
	  sum2=sum2+beta/theta;
	
	if(gamma<0) 
	  sum3=sum3-gamma/theta;
	else 
	  sum3=sum3+gamma/theta; 
...
*/
	...
	/*marker.pose.position.x = avgx/count; 
	marker.pose.position.y = avgy/count; 
	marker.pose.position.z = avgz/count;
